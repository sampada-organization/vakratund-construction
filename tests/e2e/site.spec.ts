...
test('home states the work and the real phone', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('link', { name: 'Further: Road construction' })).toBeVisible();
  await expect(page.locator('.stats').getByText('Transit mixer')).toBeVisible();
  ...
  await expect(page.getByRole('link', { name: 'Further: Foundations' })).toBeVisible();
